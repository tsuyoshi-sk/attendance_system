...
class PunchUseCase:
    ...
    def __init__(
        self, employee_repo, punch_repo, attendance_service: AttendanceService
    ):
    ...
    async def execute_punch(
        self, card_id: str, punch_type: str, timestamp: Optional[datetime] = None
    ) -> Dict[str, Any]:
        """
        打刻実行のメインロジック
        ...
        try:
            ...
            logger.info(f"打刻成功: 従業員ID={employee.id}, タイプ={punch_type}, 時刻={timestamp}")
    ...
    async def _validate_punch_type(
        ...
        if punch_type == "in":
            if latest_punch and latest_punch.punch_type in ["in", "in_break"]:
                return {"valid": False, "error": "既に出勤済みです", "error_code": "ALREADY_IN"}

        elif punch_type == "out":
            if not latest_punch or latest_punch.punch_type not in ["in", "in_break"]:
                return {"valid": False, "error": "出勤打刻が必要です", "error_code": "NOT_IN"}
