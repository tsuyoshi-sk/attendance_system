...
class SecurityLogFilter:
    ...
    SENSITIVE_FIELDS = {
        'password', 'token', 'api_key', 'secret', 'card_id', 'idm',
    ...
    def filter_dict(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            ...
            if any(sensitive in key.lower() for sensitive in SecurityLogFilter.SENSITIVE_FIELDS):
                ...
                filtered[key] = [
                    SecurityLogFilter.filter_dict(item) if isinstance(item, dict) else item
                    for item in value
                ]
            else:
                filtered[key] = value
...
class EnhancedLogger:
    ...
    def _configure_python_logging(self, log_level: str, enable_console: bool, enable_file: bool):
...
def log_api_request(method: str, path: str, status_code: int, response_time_ms: float):
    """Log API request"""
    enhanced_logger.performance.log_api_request(method, path, status_code, response_time_ms)
...
def log_security_action(action: str):
    ...
    def decorator(func):
        ...
        def sync_wrapper(*args, **kwargs):
            ...
            try:
                result = func(*args, **kwargs)
                asyncio.create_task(enhanced_logger.audit.log_audit_event(
                    category="security",
                    action=action,
                    actor=actor,
                    result="success",
