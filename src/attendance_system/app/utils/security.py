    ...
    def sanitize_string(cls, value: str, max_length: int = 1000) -> str:
        ...
        for pattern in cls.SQL_INJECTION_PATTERNS:
            if re.search(pattern, value, re.IGNORECASE):
                log_security_event(
                    "SQL_INJECTION_ATTEMPT", details=f"Pattern detected: {pattern}"
                )
        ...
        for pattern in cls.XSS_PATTERNS:
            if re.search(pattern, value, re.IGNORECASE):
                log_security_event(
                    "XSS_ATTEMPT", details=f"Pattern detected: {pattern}"
                )
                raise SecurityError("危険な入力が検出されました")
...
class RateLimiter:
    ...
    def check_rate_limit(
        ...
        self._attempts[key] = [
            (ts, count) for ts, count in self._attempts[key] if ts > cutoff
        ]
...
class CryptoUtils:
    ...
    def hash_idm(idm: str) -> str:
        ...
        Returns:
            str: ハッシュ化されたIDm
        """
        return hashlib.sha256(f"{idm}{config.IDM_HASH_SECRET}".encode()).hexdigest()
