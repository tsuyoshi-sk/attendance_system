...
class OfflineQueueManager:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        # Slack通知の設定
        if (
            hasattr(config, "SLACK_ENABLED")
            and config.SLACK_ENABLED
            and hasattr(config, "SLACK_TOKEN")
            and config.SLACK_TOKEN
        ):
    ...
    def mark_as_synced(self, record_id: int) -> bool:
        ...
        try:
            with self.db_lock:
                with sqlite3.connect(self.db_path) as conn:
                    conn.execute(
                        "DELETE FROM offline_punches WHERE id = ?", (record_id,)
                    )
                    conn.commit()
                    return True
        except Exception as e:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                total = conn.execute("SELECT COUNT(*) FROM offline_punches").fetchone()[
                    0
                ]
