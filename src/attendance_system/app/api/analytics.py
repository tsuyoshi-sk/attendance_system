...
async def get_statistics(
    ...
    try:
        ...
        service = AnalyticsService(db)
        stats = await service.get_statistics(period=period, year=year, month=month)
        return stats
...
async def get_work_hours_trend(
    months: int = 6, employee_id: Optional[str] = None, db: Session = Depends(get_db)
) -> ChartDataResponse:
    """
    労働時間トレンドチャートデータを取得
    ...
    try:
        ...
        chart_data = await service.get_work_hours_trend(
            months=months, employee_id=employee_id
        )
        return chart_data
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
async def get_overtime_distribution(
    ...
    try:
        service = AnalyticsService(db)
        chart_data = await service.get_overtime_distribution(year=year, month=month)
        return chart_data
    except Exception as e:
...
async def get_attendance_rate_chart(
    months: int = 6, db: Session = Depends(get_db)
) -> ChartDataResponse:
    """
    出勤率チャートデータを取得
    ...
    try:
        service = AnalyticsService(db)
        chart_data = await service.get_attendance_rate_trend(months=months)
        return chart_data
    except Exception as e:
...
async def get_current_alerts(db: Session = Depends(get_db)) -> List[Dict[str, Any]]:
    """
    現在のアラートを取得
...
async def get_realtime_summary(db: Session = Depends(get_db)) -> Dict[str, Any]:
    """
    リアルタイム勤怠状況サマリーを取得
