...
class PunchRequest(BaseModel):
    ...
    def validate_punch_type(cls, v):
        try:
            ...
            raise ValueError(
                f"Invalid punch_type: {v}. Valid values are: {[e.value for e in PunchType]}"
            )
        return v
...
async def create_punch(
    request: PunchRequest, db: AsyncSession = Depends(get_async_db)
) -> PunchResponse:
    """
    打刻を記録する（非同期版）
    ...
    try:
        ...
        logger.info(
            f"Punch created successfully: employee_id={result['employee']['id']}"
        )
...
async def get_punch_status(
    employee_id: int, db: AsyncSession = Depends(get_async_db)
) -> Dict[str, Any]:
    """
    従業員の現在の打刻状況を取得（非同期版）
...
async def create_batch_punches(
    punches: List[PunchRequest], db: AsyncSession = Depends(get_async_db)
) -> Dict[str, Any]:
    """
    複数の打刻を一括処理（非同期版）
    ...
    if len(punches) > 100:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="一度に処理できる打刻は100件までです"
        )
    ...
    for i, punch_request in enumerate(punches):
        try:
            ...
            errors.append(
                {"index": i, "request": punch_request.dict(), "error": str(e)}
            )
