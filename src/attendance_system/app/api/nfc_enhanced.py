...
class RetryHandler:
    ...
    def record_retry(self, scan_id: str, attempt: int):
        ...
        self.retry_storage[scan_id] = {
            ...
            "created_at": self.retry_storage.get(scan_id, {}).get(
    ...
    try:
        ...
        if request.retry_count > 0:
            if not await retry_handler.should_retry(
                request.scan_id, request.retry_count
            ):
                raise HTTPException(
        ...
        result = await process_nfc_scan(
            scan_request=request, sanitized_card_data=sanitized_card_data, db=db
        )
        ...
        background_tasks.add_task(
            broadcast_scan_update, client_id=request.client_id, result=result
        )
...
async def batch_nfc_scan(
    ...
    if len(batch_request.scans) > 10:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Maximum 10 scans per batch"
        )
    ...
    background_tasks.add_task(
        log_batch_analytics, batch_id=batch_request.batch_id, results=scan_results
    )
...
async def get_scan_status(scan_id: str, req: Request, db: Session = Depends(get_db)):
    """
...
async def validate_nfc_card(card_data: Dict[str, Any], req: Request):
    """
...
async def nfc_websocket_endpoint(
    ...
    connected = await enhanced_connection_manager.optimized_connect(
        websocket=websocket, client_id=client_id, metadata=metadata
    )
...
async def process_nfc_scan(
    scan_request: NFCScanRequest, sanitized_card_data: Dict[str, Any], db: Session
) -> Dict[str, Any]:
    ...
    try:
        ...
        # Look up employee
        employee = db.query(Employee).filter(Employee.card_id == card_id_hash).first()
...
async def process_nfc_scan_async(
    scan_request: NFCScanRequest, db: Session
) -> NFCScanResult:
...
async def log_scan_analytics(
    request: NFCScanRequest, result: Dict[str, Any], processing_time: float
):
...
async def log_batch_analytics(batch_id: str, results: List[NFCScanResult]):
    ...
    analytics_data = {
        ...
        "avg_processing_time_ms": total_processing_time / len(results)
        if results
        else 0,
        "timestamp": datetime.now().isoformat(),
    }
...
async def broadcast_scan_update(client_id: str, result: Dict[str, Any]):
    ...
    await enhanced_connection_manager.broadcast(
        update_message, exclude={client_id}  # Don't send back to originator
    )
