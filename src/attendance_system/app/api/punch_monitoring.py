...
async def punch_monitor_websocket(websocket: WebSocket, db: Session = Depends(get_db)):
    """
    リアルタイム打刻監視WebSocket
    ...
    try:
        ...
        # 定期更新タスクを開始
        update_task = asyncio.create_task(periodic_updates(websocket, db))
...
async def get_active_punches(db: Session) -> List[Dict[str, Any]]:
    ...
    recent_punches = (
        ...
        .join(Employee)
        .filter(PunchRecord.punch_time >= five_minutes_ago)
...
async def get_performance_metrics(db: Session) -> Dict[str, Any]:
    ...
    total_punches = (
        ...
        .filter(PunchRecord.punch_time >= one_hour_ago)
        .scalar()
    )

    # タイプ別集計
    type_counts = (
        db.query(PunchRecord.punch_type, func.count(PunchRecord.id))
        .filter(PunchRecord.punch_time >= one_hour_ago)
        .group_by(PunchRecord.punch_type)
        .all()
    )
...
async def check_real_time_anomalies(db: Session) -> List[Dict[str, Any]]:
    ...
    latest_punches = (
        db.query(PunchRecord)
        .filter(PunchRecord.punch_time >= datetime.now() - timedelta(minutes=1))
        .all()
    )

    anomalies = []
    for punch in latest_punches:
        detected = await anomaly_detector.detect_anomalies(
            punch, check_historical=False
        )
        if detected:
            anomalies.extend(
                [
                    {"punch_id": punch.id, "employee_id": punch.employee_id, **anomaly}
...
async def get_employee_punch_status(db: Session, employee_id: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "today_punches": [
            {"type": p.punch_type, "time": p.punch_time.isoformat()}
...
async def get_department_summary(db: Session, department: str = None) -> Dict[str, Any]:
    ...
    # 各従業員の状態を集計
    status_counts = {"in_office": 0, "outside": 0, "left": 0, "not_in": 0}
    ...
    return {
        ...
        "attendance_rate": (status_counts["in_office"] + status_counts["outside"])
        / len(employees)
        if employees
        else 0,
    }
...
async def get_system_summary(db: Session) -> Dict[str, Any]:
    ...
    today_stats = (
        ...
        .filter(func.date(PunchRecord.punch_time) == today)
        .first()
    )
...
async def get_active_employee_count(db: Session) -> int:
    """アクティブな従業員数を取得"""
    return db.query(func.count(Employee.id)).filter(Employee.is_active == True).scalar()
...
async def get_dashboard_metrics(db: Session = Depends(get_db)):
    ...
    # アクティブな従業員数（本日打刻した人数）
    active_employees = (
        db.query(func.count(func.distinct(PunchRecord.employee_id)))
        .filter(func.date(PunchRecord.punch_time) == today)
        .scalar()
    )
...
async def get_live_feed(limit: int = 20, db: Session = Depends(get_db)):
    """
    ライブフィード（最新の打刻）
