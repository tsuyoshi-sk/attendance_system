...
from backend.app.utils.auth_utils import (
    get_current_user_or_bypass,
    require_permission_or_bypass,
)
...
async def get_employees(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="従業員一覧の取得に失敗しました"
        )
...
async def create_employee(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="従業員の作成に失敗しました"
        )
...
async def update_employee(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="従業員の更新に失敗しました"
        )
...
async def delete_employee(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="従業員の削除に失敗しました"
        )


@router.post(
    "/employees/{employee_id}/cards",
...
async def add_employee_card(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="カードの追加に失敗しました"
        )
...
async def get_employee_cards(
    ...
    try:
        ...
        return CardListResponse(
            success=True, data=card_responses, total=len(card_responses)
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="カード一覧の取得に失敗しました"
        )
...
async def delete_card(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="カードの削除に失敗しました"
        )


# 従来のカード登録エンドポイント（後方互換性のため残す）
@router.post(
    "/employees/{employee_id}/card", response_model=Dict[str, Any], deprecated=True
)
async def register_card_legacy(
    ...
    card_data = CardCreate(card_idm_hash=idm_hash, card_nickname="メインカード")

    try:
        service = EmployeeService(db)
        card = await service.add_employee_card(employee_id, card_data)

        return {"message": "カードの登録が完了しました", "card": card.to_dict()}
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="カードの登録に失敗しました"
        )
