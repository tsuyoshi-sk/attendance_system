...
async def get_dashboard_overview(db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {
            ...
            "performance": {
                "cpu_usage": monitoring_summary["metrics"]
                .get("system.cpu.usage", {})
                .get("mean", 0),
                "memory_usage": monitoring_summary["metrics"]
                .get("system.memory.usage", {})
                .get("mean", 0),
                "response_time_ms": monitoring_summary["metrics"]
                .get("websocket.response_time.avg", {})
                .get("mean", 0),
                "throughput_rps": perf_stats.get("nfc_batch_processing", {}).get(
                    "count", 0
                )
                / 60,  # per minute to per second
            ...
            "alerts": {
                ...
                "critical_alerts": len(
                    [
                        a
                        for a in monitoring_summary["alerts"]["active"]
...
async def get_realtime_metrics():
    ...
    try:
        ...
        # Get latest metrics
        cpu_stats = await system_monitor.metrics_collector.get_metric_stats(
            "system.cpu.usage", 60
        )
        memory_stats = await system_monitor.metrics_collector.get_metric_stats(
            "system.memory.usage", 60
        )
        network_sent = await system_monitor.metrics_collector.get_metric_stats(
            "system.network.bytes_sent", 60
        )
        network_recv = await system_monitor.metrics_collector.get_metric_stats(
            "system.network.bytes_recv", 60
        )
...
async def get_historical_metrics(
    ...
    try:
        # Get metric series
        metric_series = await system_monitor.metrics_collector.get_metric_series(
            metric, duration
        )
        ...
        if len(metric_series) > 1000:  # Too many points, need to aggregate
            ...
            for point in metric_series:
                if point["timestamp"] - bucket_start >= interval:
                    if current_bucket:
                        avg_value = sum(p["value"] for p in current_bucket) / len(
                            current_bucket
                        )
                        aggregated.append(
                            {"timestamp": bucket_start, "value": avg_value}
                        )
            ...
            if current_bucket:
                ...
                aggregated.append({"timestamp": bucket_start, "value": avg_value})
...
async def get_active_alerts(
    ...
    try:
        ...
        all_alerts = [
            alert.to_dict()
            for alert in system_monitor.alerts.values()
        ...
        if severity:
            all_alerts = [
                alert for alert in all_alerts if alert["severity"] == severity.upper()
            ]
...
async def get_websocket_dashboard():
    ...
    try:
        ...
        for conn in connections:
            ...
            connections_by_device[device_type] = (
                connections_by_device.get(device_type, 0) + 1
            )
            connections_by_version[app_version] = (
                connections_by_version.get(app_version, 0) + 1
            )
        ...
        return {
            "overview": {
                ...
                "avg_connection_duration": sum(
                    c["connected_duration"] for c in connections
                )
                / len(connections)
                if connections
                else 0,
    ...
    try:
        ...
        if system_monitor.redis_client:
            await system_monitor.redis_client.hset(
                "alerts:active", alert_id, json.dumps(alert.to_dict())
            )
...
async def export_metrics(
    ...
    try:
        ...
        for metric_name in metric_names:
            series = await system_monitor.metrics_collector.get_metric_series(
        ...
        if format.lower() == "csv":
            ...
            writer = csv.DictWriter(
                output, fieldnames=["metric", "timestamp", "value", "tags"]
            )
            writer.writeheader()
...
async def get_system_health():
    ...
    try:
        ...
        try:
            ...
            ws_health = (
                "healthy"
                if ws_metrics["system"]["connection_usage_percent"] < 90
                else "warning"
            )
        ...
        try:
            ...
            health_status["components"]["optimizer"] = {
                ...
                "details": {
                    ...
                    "total_processed": sum(
                        stats["count"] for stats in optimizer_stats.values()
                    ),
                },
            }
        except Exception as e:
