...
async def generate_daily_report(
    request: DailyReportRequest, db: Session = Depends(get_db)
    ...
    try:
        ...
        reports = await service.generate_daily_reports(
            target_date=request.target_date, employee_ids=request.employee_ids
        )
        return reports
...
async def get_daily_report(
    target_date: date, employee_id: Optional[str] = None, db: Session = Depends(get_db)
) -> List[DailyReportResponse]:
    """
    特定日の日次レポートを取得
    ...
    try:
        ...
        reports = await service.generate_daily_reports(
            target_date=target_date, employee_ids=employee_ids
        )
        return reports
    except Exception as e:
...
async def get_employee_daily_reports(
    employee_id: str, from_date: date, to_date: date, db: Session = Depends(get_db)
) -> List[DailyReportResponse]:
    """
    従業員の期間内日次レポートを取得
    ...
    try:
        ...
        while current_date <= to_date:
            daily_reports = await service.generate_daily_reports(
                target_date=current_date, employee_ids=[employee_id]
            )
...
async def generate_monthly_report(
    request: MonthlyReportRequest, db: Session = Depends(get_db)
    ...
    try:
        ...
        reports = await service.generate_monthly_reports(
            year=request.year, month=request.month, employee_ids=request.employee_ids
        )
        return reports
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
async def get_employee_monthly_report(
    employee_id: str, year: int, month: int, db: Session = Depends(get_db)
) -> MonthlyReportResponse:
    """
    従業員の月次レポートを取得
    ...
    try:
        service = ReportService(db)
        reports = await service.generate_monthly_reports(
            year=year, month=month, employee_ids=[employee_id]
        )

        if not reports:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="指定された従業員の月次レポートが見つかりません"
            )
...
async def export_daily_csv(
    ...
    try:
        ...
        csv_content = await export_service.export_daily_csv(
            from_date=from_date, to_date=to_date, employee_ids=employee_ids
        )
...
async def export_payroll_csv(
    ...
    try:
        export_service = ExportService(db)
        csv_content = await export_service.export_payroll_csv(year=year, month=month)
