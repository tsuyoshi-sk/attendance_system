...
class PerformanceMetrics:
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            "response_time": {
                "avg": sum(response_times_list) / len(response_times_list)
                ...
                "p95": sorted(response_times_list)[int(len(response_times_list) * 0.95)]
                if len(response_times_list) > 1
                else 0,
                "p99": sorted(response_times_list)[int(len(response_times_list) * 0.99)]
                if len(response_times_list) > 1
                else 0,
            },
            "connection_time": {
                "avg": sum(connection_times_list) / len(connection_times_list)
                if connection_times_list
                else 0,
            },
...
class EnhancedNFCConnectionManager:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.redis_pool = redis.ConnectionPool.from_url(
                self.redis_url, max_connections=100, decode_responses=True
            )
    ...
    async def _process_message_batch(self, batch: List[Dict[str, Any]]):
        ...
        if broadcasts:
            ...
            self.performance_metrics.increment_message_count(
                f"batch_broadcast_{len(broadcasts)}"
            )
    ...
    async def connection_health_check(self):
        ...
        for client_id in stale_connections:
            try:
                await self.send_personal_message(
                    client_id, {"type": "ping", "timestamp": now.isoformat()}
                )
            except Exception:
                await self.disconnect(client_id)
    ...
    async def _trigger_auto_scaling(self):
        ...
        logger.warning(
            f"Auto-scaling triggered. Current connections: {len(self.active_connections)}/{self.max_connections}"
        )
        ...
        if self.redis_client:
            try:
                ...
                redis_metrics = {
                    ...
                    "redis_used_memory_mb": redis_info.get("used_memory", 0)
                    / 1024
                    / 1024,
                    "redis_total_commands": redis_info.get(
