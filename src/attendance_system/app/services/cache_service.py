...
class CacheService:
    ...
    async def get(
        self, key: str, default: Any = None, deserializer: Callable = None
    ) -> Any:
        """
        キャッシュから値を取得
    ...
    async def set(
        ...
        try:
            ...
            if self.redis_client:
                ...
                # ローカルキャッシュに保存
                expires_at = (
                    datetime.now() + timedelta(seconds=ttl) if ttl else datetime.max
                )
                self.local_cache[full_key] = {"value": value, "expires_at": expires_at}
                # メモリ制限（1000エントリまで）
                if len(self.local_cache) > 1000:
                    # 最も古いエントリを削除
                    oldest_key = min(
                        self.local_cache.keys(),
                        key=lambda k: self.local_cache[k].get(
