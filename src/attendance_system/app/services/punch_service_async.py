...
class AsyncPunchService:
    ...
    async def create_punch(
        ...
        async with DatabaseTransaction() as transaction:
            async with transaction.session() as session:
                ...
                logger.info(f"Punch created: employee_id={employee_id}, type={punch_type.value}")
    ...
    async def _validate_punch_sequence(
        ...
        if latest_punch:
            ...
            elif latest_type == PunchType.OUT and punch_type in [PunchType.OUT, PunchType.OUTSIDE, PunchType.RETURN]:
                raise ValueError("既に退勤済みです")
            elif latest_type == PunchType.OUTSIDE and punch_type == PunchType.OUTSIDE:
                raise ValueError("既に外出中です")
            elif latest_type in [PunchType.IN, PunchType.RETURN] and punch_type == PunchType.RETURN:
    ...
    async def _get_available_punch_types(self, latest_punch: Optional[PunchRecord]) -> List[str]:
        """利用可能な打刻種別を取得"""
        if not latest_punch:
    ...
    async def _punch_to_dict(self, punch: PunchRecord) -> Dict[str, Any]:
        ...
        return {
            ...
            "punch_type_display": self._get_punch_type_display(PunchType(punch.punch_type)),
