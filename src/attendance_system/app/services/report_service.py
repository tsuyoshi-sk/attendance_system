...
class ReportService:
    ...
    async def generate_daily_reports(
        self, target_date: date, employee_ids: Optional[List[str]] = None
    ...
    async def _generate_employee_daily_report(
        ...
        punch_records = [
            PunchRecordResponse(
                punch_type=punch.punch_type, timestamp=punch.punch_time, processed=True
            )
    ...
    async def _generate_employee_monthly_report(
        ...
        while current_date <= last_day:
            daily_report = await self._generate_employee_daily_report(
        ...
        monthly_summary = MonthlySummaryData(
            ...
            overtime_hours=self.time_calculator.round_monthly_overtime(
    ...
    async def generate_daily_summary(
        self, target_date: date, employee_id: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """
        日次勤怠集計を生成
        ...
        if clock_in and clock_out:
            ...
            for break_period in breaks:
                if break_period["start"] and break_period["end"]:
                    break_duration = (
                        break_period["end"].punch_time
                        - break_period["start"].punch_time
                    )
            ...
            if summary["actual_work_minutes"] > standard_minutes:
                summary["overtime_minutes"] = (
                    summary["actual_work_minutes"] - standard_minutes
                )

        return summary

    async def export_monthly_report_csv(
        self, year: int, month: int, employee_id: Optional[int] = None
    ) -> str:
        """
        月次レポートをCSV形式でエクスポート
    ...
    async def get_monthly_statistics(
        ...
        for result in results:
            employee = (
                ...
                .filter(Employee.id == result.employee_id)
                .first()
            )
