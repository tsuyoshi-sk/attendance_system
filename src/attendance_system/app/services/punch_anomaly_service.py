...
class PunchAnomalyDetector:
    ...
    ANOMALY_PATTERNS = {
        ...
        "LONG_WORK_TIME": {"max_hours": 16, "severity": "HIGH", "description": "長時間勤務"},
        "IMPOSSIBLE_TRAVEL": {
    ...
    async def detect_anomalies(
        self, punch_record: PunchRecord, check_historical: bool = True
    ) -> List[Dict[str, any]]:
        """
        異常パターン検出
        ...
        recent_punches = (
            ...
            .filter(
                and_(
                    ...
                    >= punch_record.punch_time - timedelta(seconds=pattern["interval"]),
    ...
    async def _check_long_work_time(
        ...
        if in_punch:
            work_hours = (
                punch_record.punch_time - in_punch.punch_time
            ).total_seconds() / 3600
    ...
    async def _check_missing_punch(
        ...
        if outside_count > return_count:
            last_outside = next(
                p for p in reversed(daily_punches) if p.punch_type == PunchType.OUTSIDE
            )

            if (
                datetime.now() - last_outside.punch_time
            ).total_seconds() > 7200:  # 2時間以上
                return {
                    "type": "MISSING_PUNCH",
                    "severity": pattern["severity"],
                    ...
                    "details": {
                        ...
                        "duration_hours": round(
                            (datetime.now() - last_outside.punch_time).total_seconds()
                            / 3600,
                            2,
                        ),
                    },
            ...
            return {
                ...
                "details": {
                    ...
                    "interval_minutes": round(
                        (punch_record.punch_time - recent_in.punch_time).total_seconds()
                        / 60,
                        2,
                    ),
                },
    ...
    async def _check_irregular_pattern(
        ...
        current_minutes = (
            punch_record.punch_time.hour * 60 + punch_record.punch_time.minute
        )
    ...
    async def generate_anomaly_report(
        ...
        query = self.db.query(PunchRecord).filter(
            and_(
                PunchRecord.punch_time >= start_date, PunchRecord.punch_time <= end_date
            )
        )
        ...
        high_priority_anomalies = [
            a for a in all_anomalies if a["severity"] in ["CRITICAL", "HIGH"]
        ]
        ...
        return {
            ...
            "anomaly_rate": len(all_anomalies) / len(punch_records)
            if punch_records
            else 0,
