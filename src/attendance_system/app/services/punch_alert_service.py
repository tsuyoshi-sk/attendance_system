...
class PunchAlertService:
    ...
    async def _check_missing_out(self, employee: Employee, current_time: datetime):
        ...
        # 退勤打刻を確認
        out_punch = self.db.query(PunchRecord).filter(
            and_(
                PunchRecord.employee_id == employee.id,
        ...
        if not out_punch:
            ...
            delay_minutes = int((current_time - scheduled_out_time).total_seconds() / 60)
    ...
    async def _check_missing_return(self, employee: Employee, current_time: datetime):
        """戻り打刻漏れチェック"""
        # 本日の外出打刻を取得
        today_outside = self.db.query(PunchRecord).filter(
            and_(
                PunchRecord.employee_id == employee.id,
    ...
    async def _check_consecutive_missing(self, employee: Employee, current_time: datetime):
        """連続欠勤チェック"""
        ...
        for i in range(threshold_days):
            ...
            punch_exists = self.db.query(PunchRecord).filter(
                and_(
                    PunchRecord.employee_id == employee.id,
                    PunchRecord.punch_type == PunchType.IN,
