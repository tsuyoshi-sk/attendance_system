    ...
    async def record_metric(self, name: str, value: float, tags: Dict[str, str] = None):
        ...
        async with self._lock:
            metric_point = MetricPoint(
                timestamp=time.time(), value=value, tags=tags or {}
            )
...
class SystemMonitor:
    ...
    async def _collect_system_metrics_once(self):
        ...
        for i, percent in enumerate(cpu_per_core):
            await self.metrics_collector.record_metric(
                "system.cpu.core.usage", percent, tags={"core": str(i)}
            )
        ...
        await self.metrics_collector.record_metric(
            "process.cpu.usage", process.cpu_percent()
        )
        await self.metrics_collector.record_metric(
        ...
        await self.metrics_collector.record_metric(
            "process.fds", process.num_fds() if hasattr(process, "num_fds") else 0
        )
    ...
    async def _collect_application_metrics_once(self):
        ...
        if enhanced_connection_manager:
            ...
            if perf_data["response_time"]["avg"] > 0:
                await self.metrics_collector.record_metric(
                    "websocket.response_time.avg", perf_data["response_time"]["avg"]
                )
                await self.metrics_collector.record_metric(
                    "websocket.response_time.p95", perf_data["response_time"]["p95"]
                )
                await self.metrics_collector.record_metric(
                    "websocket.response_time.p99", perf_data["response_time"]["p99"]
                )
            ...
            if "redis" in perf_data:
                ...
                await self.metrics_collector.record_metric(
                    "redis.memory_used_mb", perf_data["redis"]["redis_used_memory_mb"]
                )
        ...
        if self.redis_client:
            try:
                ...
                for endpoint, count in api_metrics.items():
                    await self.metrics_collector.record_metric(
                        "api.requests", float(count), tags={"endpoint": endpoint}
                    )
    ...
    async def analyze_performance(self) -> Dict[str, Any]:
        ...
        cpu_stats = await self.metrics_collector.get_metric_stats(
            "system.cpu.usage", 300
        )
        memory_stats = await self.metrics_collector.get_metric_stats(
            "system.memory.usage", 300
        )
        response_time_stats = await self.metrics_collector.get_metric_stats(
            "websocket.response_time.avg", 300
        )
    ...
    async def _handle_anomaly(self, anomaly: Dict[str, Any]):
        ...
        alert = Alert(
            ...
            + (self.anomaly_detector.sensitivity * anomaly["std"]),
    ...
    async def _create_alert(self, alert: Alert):
        ...
        if self.redis_client:
            await self.redis_client.hset(
                "alerts:active", alert.id, json.dumps(alert.to_dict())
            )
    ...
    async def get_monitoring_summary(self) -> Dict[str, Any]:
        ...
        # Get active alerts
        active_alerts = [
            alert.to_dict() for alert in self.alerts.values() if not alert.resolved
        ]
    ...
    def _determine_system_status(
        self, performance: Dict[str, Any], active_alerts: List[Dict[str, Any]]
    ) -> str:
