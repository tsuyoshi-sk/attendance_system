...
class SecurityManager:
    ...
    def _validate_security_keys(self) -> None:
        ...
        if len(secret_key) < 64:
            raise ValueError('SECRET_KEY must be at least 64 characters for ASVS Level 2')
        if len(jwt_key) < 64:
            raise ValueError('JWT_SECRET_KEY must be at least 64 characters for ASVS Level 2')
        if len(idm_secret) < 64:
            raise ValueError('IDM_HASH_SECRET must be at least 64 characters for ASVS Level 2')
