...
class TestSecurityAudit:
        ...
        for file_path in project_files:
            try:
                ...
                for pattern in dangerous_patterns:
                    ...
                    # テスト用の値は除外
                    filtered_matches = [m for m in matches if 'test' not in m.lower() and 'your-' not in m.lower()]
                    if filtered_matches:
    ...
    def test_environment_variable_security(self):
        ...
        for var in required_vars:
            ...
            for pattern in weak_patterns:
                assert pattern.lower() not in value.lower(), f"{var} contains weak pattern: {pattern}"
    ...
    def test_crypto_key_strength(self):
        ...
        for key_name, key_value in keys_to_check.items():
            ...
            assert unique_chars >= 20, f"{key_name} low entropy: {unique_chars} unique chars"
    ...
    def test_security_headers_implementation(self):
        ...
        for header, expected_value in required_headers.items():
            ...
            assert expected_value in headers[header], f"Weak {header}: {headers[header]}"
    ...
    def test_authentication_implementation(self):
        ...
        assert '$12$' in hashed or '$13$' in hashed or '$14$' in hashed, "bcrypt cost should be 12+"
...
class TestSecurityPerformance:
    ...
    def test_password_hashing_performance(self):
        ...
        assert hash_time < 5.0, f"Password hashing too slow: {hash_time}s for 10 operations"
    ...
    def test_encryption_performance(self):
        ...
        assert crypto_time < 2.0, f"Encryption too slow: {crypto_time}s for 100 operations"
...
class TestSecurityAudit:
    ...
    def test_no_hardcoded_secrets(self):
        """ハードコードされた秘密情報の検出テスト"""
        project_files = list(Path("backend").glob("**/*.py")) + list(Path("src").glob("**/*.py"))
        ...
        for file_path in project_files:
            try:
                ...
                for pattern in dangerous_patterns:
                    ...
                    if matches:
                        # 除外パターン（テストファイルやサンプル）
                        if any(exclude in str(file_path) for exclude in ['test_', 'sample', 'example']):
                            continue
    ...
    def test_sql_injection_prevention(self):
        """SQLインジェクション対策チェック"""
        python_files = list(Path("backend").glob("**/*.py")) + list(Path("src").glob("**/*.py"))
        ...
        for file_path in python_files:
            try:
                ...
                for pattern in dangerous_sql_patterns:
                    matches = re.findall(pattern, content, re.IGNORECASE)
                    if matches:
                        violations.append(f"{file_path}: Potential SQL injection risk: {matches}")
            except Exception:
                continue
        
        assert len(violations) == 0, f"Potential SQL injection vulnerabilities: {violations}"
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.ExceptHandler):
                        ...
                        if node.type is None:
                            violations.append(f"{file_path}:line {node.lineno}: Bare except clause found")
                        ...
                        if (node.type and 
                            ...
                            violations.append(f"{file_path}:line {node.lineno}: Silent exception handling")
            except Exception:
                continue
        
        # 一部の violations は許容（テストファイルなど）
        filtered_violations = [v for v in violations if 'test_' not in v]
        assert len(filtered_violations) <= 5, f"Too many error handling issues: {filtered_violations}"
        ...
        for file_path in python_files:
            try:
                ...
                if 'import random' in content or 'from random import' in content:
                    ...
                    if any(keyword in str(file_path) for keyword in ['security', 'auth', 'token', 'crypto']):
                        violations.append(f"{file_path}: Using non-cryptographic random in security context")
            except Exception:
                continue
    ...
    def test_input_validation_patterns(self):
        ...
        for file_path in python_files:
            ...
            try:
                ...
                if re.search(r'@\w*\.post\s*\(', content) or re.search(r'@\w*\.put\s*\(', content):
                    # Pydantic モデルまたは validation の使用をチェック
                    if not (re.search(r'from.*pydantic', content) or 
                           re.search(r'from.*schemas', content) or
                           re.search(r':\s*\w+Schema', content)):
                        missing_validation.append(f"{file_path}: POST/PUT endpoint without apparent validation")
            except Exception:
                continue
        
        # API ファイルが少ない場合は許容
        assert len(missing_validation) <= 3, f"Endpoints without validation: {missing_validation}"
        ...
        for file_path in python_files:
            try:
                ...
                for pattern in log_patterns:
                    matches = re.findall(pattern, content, re.IGNORECASE)
                    if matches:
                        violations.append(f"{file_path}: Potential sensitive data in logs: {matches}")
            except Exception:
                continue
...
class TestSecurityConfiguration:
    ...
    def test_encryption_key_strength(self):
        ...
        assert unique_chars > 20, f"SECRET_KEY has low entropy: {unique_chars} unique chars"