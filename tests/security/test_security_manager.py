...
from backend.app.security.enhanced_auth import SecurityManager, TokenManager, IntrusionDetector, SecurityAuditor
...
class TestSecurityManager:
    ...
    async def test_validate_nfc_request_valid(self, security_manager):
        ...
        # モックで正常な検証を設定
        with patch.object(security_manager.intrusion_detector, 'is_client_blocked', return_value=False), \
             patch.object(security_manager.validator, 'validate_nfc_scan_request', return_value=True), \
             patch.object(security_manager.intrusion_detector, 'detect_suspicious_activity', return_value=[]), \
             patch.object(security_manager.intrusion_detector, 'track_request', return_value=None):
            
            result = await security_manager.validate_nfc_request(valid_request, client_id)
    ...
    async def test_validate_nfc_request_blocked_client(self, security_manager):
        ...
        # クライアントをブロック状態に設定
        with patch.object(security_manager.intrusion_detector, 'is_client_blocked', return_value=True):
            result = await security_manager.validate_nfc_request(valid_request, client_id)
    ...
    async def test_validate_nfc_request_invalid_data(self, security_manager):
        ...
        with patch.object(security_manager.intrusion_detector, 'is_client_blocked', return_value=False), \
             patch.object(security_manager.validator, 'validate_nfc_scan_request', return_value=False), \
             patch.object(security_manager.intrusion_detector, 'track_request', return_value=None):
            
            result = await security_manager.validate_nfc_request(invalid_request, client_id)
            assert result is False
    ...
    async def test_authenticate_websocket_success(self, security_manager):
        ...
        with patch.object(security_manager.token_manager, 'validate_websocket_token', return_value=expected_payload), \
             patch.object(security_manager.intrusion_detector, 'is_client_blocked', return_value=False), \
             patch.object(security_manager.intrusion_detector, 'track_request', return_value=None), \
             patch.object(security_manager.security_auditor, 'log_authentication_event', return_value=None):
    ...
    async def test_authenticate_websocket_blocked_client(self, security_manager):
        ...
        with patch.object(security_manager.token_manager, 'validate_websocket_token', return_value={"user_id": "user_123"}), \
             patch.object(security_manager.intrusion_detector, 'is_client_blocked', return_value=True):
    ...
    async def test_get_security_status(self, security_manager):
        ...
        with patch.object(security_manager.security_auditor, 'get_audit_trail', return_value=mock_events), \
             patch.object(security_manager.intrusion_detector, 'redis_client') as mock_redis:
    ...
    async def test_full_nfc_workflow(self):
        ...
        with patch('redis.asyncio.Redis') as mock_redis:
            ...
            for idm in iphone_idms:
                ...
                # NFC要求検証テスト
                with patch.object(sm.intrusion_detector, 'is_client_blocked', return_value=False), \
                     patch.object(sm.validator, 'validate_nfc_scan_request', return_value=True), \
                     patch.object(sm.intrusion_detector, 'detect_suspicious_activity', return_value=[]), \
                     patch.object(sm.intrusion_detector, 'track_request', return_value=None):
