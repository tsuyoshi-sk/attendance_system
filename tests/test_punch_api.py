...
def test_employee():
    ...
    idm_hash = hashlib.sha256(f"{test_idm}{config.IDM_HASH_SECRET}".encode()).hexdigest()
...
class TestPunchAPI:
    ...
    def test_daily_limit(self, client, test_employee):
        ...
        for i in range(3):
            ...
            client.post("/api/v1/punch/", json={
                "punch_type": "OUTSIDE",
                "card_idm": idm_hash,
                "timestamp": (base_time + timedelta(hours=i+1, minutes=30)).isoformat()
            })
        ...
        # 4回目の外出（制限超過）
        response = client.post("/api/v1/punch/", json={
            "punch_type": "OUTSIDE",
            "card_idm": idm_hash,
    ...
    def test_offline_mode(self, mock_add_punch, client):
        ...
        # ネットワークエラーをシミュレート
        with patch('backend.app.services.punch_service.PunchService.create_punch') as mock_create:
