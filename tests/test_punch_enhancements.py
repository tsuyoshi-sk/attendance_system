...
class TestPunchAnomalyService:
    ...
    async def test_rapid_consecutive_detection(self, db, test_employee):
        ...
        # 最後の打刻をチェック
        last_punch = (
            db.query(PunchRecord).order_by(PunchRecord.punch_time.desc()).first()
        )
...
class TestErrorRecovery:
    ...
    async def test_intelligent_recovery(self):
        ...
        # USBエラーのコンテキスト
        context = {"device_id": "test_device", "error": "USB disconnected"}
...
class TestDeviceMonitor:
    ...
    async def test_predictive_maintenance(self):
        ...
        # 健全性履歴を作成
        for i in range(15):
            monitor.health_history["test_device"] = monitor.health_history.get(
                "test_device", []
            )
