...
class TestCompleteAttendanceWorkflow:
    ...
    def test_complete_attendance_workflow(self, client: TestClient, db: Session):
        ...
        response = client.post(
            f"/api/v1/admin/employees/{employee_id}/card",
            json=card_data
        )
        ...
        employee_report = next(
            (r for r in daily_report if r["employee_name"] == "統合テスト太郎"),
            None
        )
    ...
    def test_multi_employee_workflow(self, client: TestClient, db: Session):
        ...
        for i in range(10):
            ...
            response = client.post(
                f"/api/v1/admin/employees/{employee['id']}/card",
                json=card_data
            )
            assert response.status_code == 200
        ...
        # 10名全員が含まれていることを確認
        multi_employees = [r for r in daily_report if r["employee_name"].startswith("マルチテスト")]
