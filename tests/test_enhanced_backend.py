...
class TestEnhancedWebSocketManager:
    ...
    async def manager(self):
        ...
        manager = EnhancedNFCConnectionManager(
            redis_url="redis://localhost:6379/15", max_connections=10  # Test DB
        )
...
class TestSecurityManager:
    ...
    def test_password_validation(self):
        ...
        assert not SecurityValidator.validate_password_strength(
            "nouppercase123!"
        )  # No uppercase
        assert not SecurityValidator.validate_password_strength(
            "NOLOWERCASE123!"
    ...
    async def test_intrusion_detection(self, security_manager):
        ...
        is_blocked = await security_manager.intrusion_detector.is_client_blocked(
            client_id
        )
        assert is_blocked
...
class TestSystemMonitor:
    ...
    async def test_performance_analysis(self, monitor):
        ...
        await monitor.metrics_collector.record_metric(
            "websocket.response_time.avg", 80.0
        )
...
class TestAsyncOptimizer:
    ...
    async def optimizer(self):
        ...
        optimizer = AsyncOptimizer(redis_url="redis://localhost:6379/15", max_workers=2)
            ...
            scan_requests.append(
                {
                    ...
                    "client_id": "test_client",
                    "card_data": {"idm": f"012345678{i:07d}", "type": "suica"},
                    "timestamp": int(time.time() * 1000),
                    "operation": "lookup",
                }
            )
...
class TestEnhancedLogger:
    ...
    def test_security_logging(self, logger):
        ...
        logger.security.log_nfc_scan_attempt(
            card_id="0123456789ABCDEF", success=True, client_id="test_client"
        )

        # Test authentication logging
        logger.security.log_authentication_attempt(
            user_id="test_user", method="password", success=True, ip_address="127.0.0.1"
        )
    ...
    def test_performance_logging(self, logger):
        ...
        logger.performance.log_websocket_message(
            message_type="scan_result", client_id="test_client", processing_time_ms=50.0
        )
    ...
    def test_context_management(self, logger):
        ...
        # Set context
        logger.set_context(
            request_id="req_123", user_id="user_456", client_id="client_789"
        )
                ...
                scan_requests.append(
                    {
                        ...
                        "card_data": {"idm": f"PERF{i:012d}", "type": "suica"},
                        "timestamp": int(time.time() * 1000),
                        "operation": "lookup",
                    }
                )
