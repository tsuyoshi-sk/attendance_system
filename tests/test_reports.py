...
def sample_punch_records(test_db, sample_employee):
    ...
    with test_db.get_session() as db:
        ...
        break_end = PunchRecord(
            ...
            punch_time=datetime.combine(
                target_date, datetime.min.time().replace(hour=13)
            ),
        )
...
class TestReportService:
    ...
    def test_generate_daily_reports(
        self, test_db, sample_employee, sample_punch_records
    ):
        """日次レポート生成のテスト"""
        with test_db.get_session() as db:
    ...
    def test_generate_monthly_reports(
        ...
        with test_db.get_session() as db:
            ...
            today = date.today()
            reports = asyncio.run(
                service.generate_monthly_reports(today.year, today.month)
            )
...
class TestReportAPI:
    ...
    def test_daily_report_endpoint(
        self, client, test_db, sample_employee, sample_punch_records
    ):
        """日次レポートAPIのテスト"""
        # データベースセッションをモック
    ...
    def test_overtime_calculation(self, test_db, sample_employee):
        ...
        with test_db.get_session() as db:
            ...
            punch_out = PunchRecord(
                ...
                punch_type=PunchType.CLOCK_OUT.value,
                punch_time=datetime.combine(
                    target_date, datetime.min.time().replace(hour=19)
                ),  # 19時
            )
