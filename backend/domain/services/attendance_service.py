...
class AttendanceService:
    ...
    def _round_time(self, time_delta: timedelta, round_minutes: int) -> timedelta:
        ...
        rounded_minutes = ((total_minutes + round_minutes // 2) // round_minutes) * round_minutes
        return timedelta(minutes=rounded_minutes)
    
    def calculate_monthly_summary(self, daily_summaries: List[WorkSummary]) -> Dict:
        """月次サマリー計算"""
        total_work_time = sum(
            (summary.work_time for summary in daily_summaries),
            timedelta(0)
        )
        
        total_overtime_normal = sum(
            (summary.overtime_normal for summary in daily_summaries),
            timedelta(0)
        )
        
        total_overtime_late = sum(
            (summary.overtime_late for summary in daily_summaries),
            timedelta(0)
        )
        
        total_night_time = sum(
            (summary.night_time for summary in daily_summaries),
            timedelta(0)
        )
