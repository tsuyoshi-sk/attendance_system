...
class EmployeeService:
    ...
    def create_employee(self, employee_data: EmployeeCreate) -> Employee:
        ...
        existing = self.db.query(Employee).filter(
            or_(
                ...
                Employee.email == employee_data.email if employee_data.email else False
            )
        ).first()
    ...
    def delete_employee(self, employee_id: int) -> bool:
        ...
        # 関連するカードも無効化
        cards = self.db.query(EmployeeCard).filter(
            EmployeeCard.employee_id == employee_id
        ).all()
        for card in cards:
    ...
    def add_employee_card(self, employee_id: int, card_data: CardCreate) -> EmployeeCard:
        ...
        # カードIDmハッシュの重複チェック
        existing = self.db.query(EmployeeCard).filter(
            EmployeeCard.card_idm_hash == card_data.card_idm_hash
        ).first()
        if existing:
    ...
    def get_employee_cards(self, employee_id: int) -> List[EmployeeCard]:
        ...
        """
        return self.db.query(EmployeeCard).filter(
            EmployeeCard.employee_id == employee_id,
    ...
    def delete_card(self, card_id: int) -> bool:
        ...
        if employee and employee.card_idm_hash == card.card_idm_hash:
            # 他のアクティブなカードを探す
            other_card = self.db.query(EmployeeCard).filter(
