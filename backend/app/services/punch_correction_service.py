...
class PunchCorrectionService:
    ...
    async def _suggest_missing_out_correction(
        ...
        if not historical_data:
            ...
            suggested_time = datetime.combine(
                work_date,
                time(int(avg_out_minutes // 60), int(avg_out_minutes % 60))
            )
        ...
        return {
            ...
            "correction_details": {
                ...
                "estimated_break_minutes": round(
                    (suggested_time - last_outside).total_seconds() / 60, 
                    2
                )
            }
        }
    ...
    async def _suggest_duplicate_correction(
        ...
        # 退勤時刻との関係で判定
        out_punch = next(
            (p for p in daily_punches if p.punch_type == PunchType.OUT), 
            None
        )
        
        if out_punch:
            work_hours_first = (out_punch.punch_time - first_punch).total_seconds() / 3600
            work_hours_second = (out_punch.punch_time - second_punch).total_seconds() / 3600
    ...
    async def auto_correct_minor_issues(
        ...
        if punch_record.punch_time > datetime.now() + timedelta(minutes=1):
            # 1分以上未来の場合は現在時刻に修正
            original_time = punch_record.punch_time
            punch_record.punch_time = datetime.now().replace(
                second=0,
                microsecond=0
            )
            corrections_made.append({
    ...
    async def _analyze_break_pattern(
        ...
        # 外出・戻りのペアを取得
        outside_punches = self.db.query(PunchRecord).filter(
            and_(
                PunchRecord.employee_id == employee_id,
        ...
        for outside_punch in outside_punches:
            # 対応する戻り打刻を探す
            return_punch = self.db.query(PunchRecord).filter(
                and_(
                    PunchRecord.employee_id == employee_id,
                    PunchRecord.punch_type == PunchType.RETURN,
                    PunchRecord.punch_time > outside_punch.punch_time,
                    func.date(PunchRecord.punch_time) == func.date(outside_punch.punch_time)
                )
            ).order_by(PunchRecord.punch_time).first()
        ...
        stdev_break = statistics.stdev(break_durations) if len(break_durations) > 1 else 0
    ...
    async def apply_correction(
        ...
        try:
            if correction["type"] == "ADD_PUNCH":
                ...
                # 打刻を削除（実際には無効化）
                target_punch = self.db.query(PunchRecord).filter(
                    and_(
                        PunchRecord.punch_time == datetime.fromisoformat(
                            correction["target_time"]
                        ),
