...
class ExportService:
    ...
    async def export_payroll_csv(
        ...
        writer.writerow([
            "従業員コード", "従業員名", "年", "月",
            "出勤日数", "総労働時間", "通常労働時間", "残業時間", "深夜時間", "休日労働時間",
            "基本給", "残業代", "深夜手当", "休日手当", "総支給額", "控除額", "差引支給額"
        ])
    ...
    async def export_attendance_summary_csv(
        self,
        year: int,
        month: int
    ) -> str:
        ...
        while current_date <= last_day:
            ...
            for report in daily_reports:
                ...
                writer.writerow([
                    ...
                    f"{report.summary.overtime_minutes / 60:.1f}時間" if report.summary.overtime_minutes > 0 else "",
                    f"{report.summary.night_minutes / 60:.1f}時間" if report.summary.night_minutes > 0 else "",
                    "",  # TODO: 遅刻時間
                    "",  # TODO: 早退時間
                    ", ".join(notes)
                ])
