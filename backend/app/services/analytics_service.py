...
from backend.app.schemas.report import (
    DashboardResponse, DashboardSummary, DashboardAlert,
    StatisticsResponse, AttendanceStats, OvertimeAnalysis, TrendAnalysis,
    ChartDataResponse
)
...
class AnalyticsService:
    ...
    async def _get_today_summary(self) -> DashboardSummary:
        ...
        # 本日出勤済み従業員数
        present_employees = self.db.query(func.count(func.distinct(PunchRecord.employee_id))).filter(
            and_(
        ...
        total_work_hours = sum(
            report.summary.actual_work_minutes / 60.0
            for report in daily_reports
        )
        
        # 平均労働時間
        average_work_hours = total_work_hours / present_employees if present_employees > 0 else 0
    ...
    async def _get_this_month_summary(self) -> Dict[str, Any]:
        ...
        monthly_stats = self.db.query(
            ...
            func.count(func.distinct(func.date(PunchRecord.punch_time))).label("unique_days")
        ).filter(
            and_(
        ...
        attendance_rate = (monthly_stats.unique_employees / (total_employees * work_days) * 100) if work_days > 0 else 0
    ...
    async def get_statistics(
        ...
        # 残業分析
        overtime_analysis = await self._calculate_overtime_analysis(start_date, end_date)
    ...
    async def _calculate_attendance_stats(
        ...
        return AttendanceStats(
            ...
            standard_deviation=statistics.stdev(work_hours_list) if len(work_hours_list) > 1 else 0
        )
    ...
    async def _calculate_overtime_analysis(
        ...
        return OvertimeAnalysis(
            ...
            average_overtime_per_employee=total_overtime / employee_count if employee_count > 0 else 0,
    ...
    async def _calculate_trend_analysis(
        ...
        if current_overtime.total_overtime_hours > prev_overtime.total_overtime_hours * 1.05:
            overtime_trend = "increasing"
        elif current_overtime.total_overtime_hours < prev_overtime.total_overtime_hours * 0.95:
    ...
    async def get_work_hours_trend(
        ...
        for i in range(months - 1, -1, -1):
            ...
            # 集計
            total_work_hours = sum(r.monthly_summary.total_work_hours for r in monthly_reports)
            total_overtime_hours = sum(r.monthly_summary.overtime_hours for r in monthly_reports)
    ...
    async def get_overtime_distribution(
        ...
        """
        # 月次レポートを生成
        monthly_reports = await self.report_service.generate_monthly_reports(year, month)
    ...
    async def get_attendance_rate_trend(
        self,
        months: int = 6
    ) -> ChartDataResponse:
        """
        ...
        for i in range(months - 1, -1, -1):
            ...
            # 出勤者数を集計
            attendance_count = self.db.query(
    ...
    async def get_current_alerts(self) -> List[DashboardAlert]:
        ...
        for report in monthly_reports:
            # 月間残業時間アラート
            if report.monthly_summary.overtime_hours >= self.alert_conditions["overtime_monthly"]:
            ...
            consecutive_overtime = await self._check_consecutive_overtime(
                report.employee_id,
                self.alert_conditions["continuous_overtime"]
            )
        ...
        for report in daily_reports:
            # 長時間残業アラート
            if report.summary.overtime_minutes >= self.alert_conditions["overtime_daily"] * 60:
                alerts.append(DashboardAlert(
                    type="daily_overtime",
                    employee_id=report.employee_id,
    ...
    async def get_realtime_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "working_count": len([e for e in working_employees if e["status"] == "working"]),
            "break_count": len([e for e in working_employees if e["status"] == "break"]),
