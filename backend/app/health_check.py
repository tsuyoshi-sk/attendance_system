...
class HealthChecker:
    ...
    def _check_punch_system(self) -> SubsystemHealth:
        ...
        try:
            # 最新の打刻記録を確認
            latest_punch = self.db.query(PunchRecord).order_by(
            ...
            # 今日の打刻数
            today_count = self.db.query(func.count(PunchRecord.id)).filter(
                func.date(PunchRecord.punch_time) == datetime.now().date()
            ).scalar()
    ...
    def _check_employee_system(self) -> SubsystemHealth:
        ...
        try:
            ...
            # カード登録済み従業員数
            with_card = self.db.query(func.count(Employee.id)).filter(
                Employee.card_idm_hash.isnot(None)
            ).scalar()
    ...
    def _check_report_system(self) -> SubsystemHealth:
        ...
        try:
            # 最新の日次集計
            latest_daily = self.db.query(DailySummary).order_by(
                DailySummary.work_date.desc()
            ).first()
            
            # 最新の月次集計
            latest_monthly = self.db.query(MonthlySummary).order_by(
            ...
            if latest_monthly:
                details["latest_monthly_summary"] = f"{latest_monthly.year}-{latest_monthly.month:02d}"
            else:
    ...
    def _check_pasori(self) -> SubsystemHealth:
        ...
        try:
            ...
            return SubsystemHealth(
                ...
                {
                    "mock_mode": False,
                    "timeout_seconds": config.PASORI_TIMEOUT
                }
            )
        except Exception as e:
