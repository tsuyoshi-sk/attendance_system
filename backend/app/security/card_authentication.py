...
class CardSecurityValidator:
    ...
    SECURITY_RULES = {
        "idm_pattern": {
            "min_length": 16,
            "max_length": 16,
            "valid_prefixes": ["01", "02", "03", "04", "05"],  # Sony FeliCa の一般的なプレフィックス
        },
        "behavior_analysis": {
    ...
    async def detect_card_forgery(
        ...
        # セキュリティスコアを計算
        security_score = await self._calculate_security_score(security_checks, card_data)
    ...
    def _check_idm_format(self, idm: str) -> Dict[str, any]:
        ...
        checks = {
            ...
            "valid_prefix": any(idm.upper().startswith(p) for p in rules["valid_prefixes"]),
    ...
    def _check_signal_pattern(self, signal_strength: float) -> Dict[str, any]:
        ...
        if signal_strength <= 0 or signal_strength > 1:
            return {"passed": False, "confidence": 0, "reason": "Invalid signal strength"}
    ...
    def _is_sequential(self, idm: str) -> bool:
        ...
        for i in range(len(idm) - 3):
            if idm[i:i+4] in ["0123", "1234", "2345", "3456", "4567", 
                              "5678", "6789", "789A", "89AB", "9ABC", 
                              "ABCD", "BCDE", "CDEF"]:
                return True
        return False
    ...
    async def behavioral_analysis(
        ...
        return {
            ...
            "recommendation": self._generate_behavior_recommendation(anomalies, risk_score)
        }
    ...
    def _calculate_usual_locations(
        ...
        for record in punch_records:
            if record.location_name:
                locations[record.location_name] = locations.get(record.location_name, 0) + 1
