...
class TokenManager:
    ...
    def generate_websocket_token(self, client_id: str, user_id: Optional[str] = None) -> str:
        """Generate secure WebSocket authentication token"""
        payload = {
            'client_id': client_id,
            'user_id': user_id,
            'exp': datetime.utcnow() + timedelta(minutes=SecurityConfig.WS_TOKEN_EXPIRE_MINUTES),
            'iat': datetime.utcnow(),
            'type': 'websocket',
    ...
    async def validate_websocket_token(self, token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            token_hash = hashlib.sha256(token.encode()).hexdigest()
            if self.redis_client:
                is_blacklisted = await self.redis_client.exists(f"blacklist:{token_hash}")
                if is_blacklisted:
...
class SecurityValidator:
    ...
    def sanitize_input(data: Any, max_length: int = 1000) -> Any:
        ...
        if isinstance(data, str):
            ...
            return {k: SecurityValidator.sanitize_input(v, max_length) for k, v in data.items()}
...
class IntrusionDetector:
    ...
    async def track_request(self, client_id: str, request_type: str, success: bool = True):
    ...
    async def _check_rate_limits(self, client_id: str, request_type: str):
        ...
        recent_requests = [
            req for req in self.request_tracker[client_id]
    ...
    async def _trigger_security_event(self, client_id: str, event_type: str, message: str):
    ...
    async def detect_suspicious_activity(self, request_data: Dict[str, Any]) -> List[str]:
...
class SecurityAuditor:
    ...
    async def log_nfc_scan_attempt(self, card_id_hash: str, client_id: str, success: bool, details: Dict[str, Any] = None):
    ...
    async def log_authentication_event(self, user_id: str, event_type: str, success: bool, details: Dict[str, Any] = None):
    ...
    async def log_security_event(self, event_type: str, severity: str, details: Dict[str, Any]):
        """Log security event"""
        event = {
            'category': 'security',
            'event_type': event_type,
    ...
    async def _store_audit_event(self, event: Dict[str, Any]):
        ...
        if self.redis_client:
            ...
            await self.redis_client.lpush(
                f"audit:daily:{date_key}",
                json.dumps(event)
            )
            await self.redis_client.expire(f"audit:daily:{date_key}", 90 * 86400)  # 90 days
    ...
    async def generate_compliance_report(self, date: datetime) -> Dict[str, Any]:
        ...
        if self.redis_client:
            raw_events = await self.redis_client.lrange(f"audit:daily:{date_str}", 0, -1)
...
class SecurityManager:
    ...
    async def authenticate_websocket(self, token: str, client_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            await self.intrusion_detector.track_request(client_id, 'ws_auth', success=True)
            ...
            await self.intrusion_detector.track_request(client_id, 'ws_auth', success=False)
            await self.security_auditor.log_authentication_event(
                'unknown',
                'websocket_auth',
                False,
    ...
    async def validate_nfc_request(self, request_data: Dict[str, Any], client_id: str) -> bool:
        ...
        if not self.validator.validate_nfc_scan_request(request_data):
            await self.intrusion_detector.track_request(client_id, 'nfc_scan', success=False)
            return False
        
        # Check for suspicious patterns
        warnings = await self.intrusion_detector.detect_suspicious_activity(request_data)
        if warnings:
